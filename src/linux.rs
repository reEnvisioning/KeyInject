...
pub(crate) fn available() -> Result<(), String> {
    fs::OpenOptions::new()
        ...
        .open("/dev/uinput")
        .map(|_| ())
        .map_err(|error| setup_error("cannot access /dev/uinput", error))
}
...
fn run_key(command: &str, key: &str) -> Result<(), String> {
    key_code(key).ok_or_else(|| format!("unknown key or button: {key}"))?;
    run_client(format!("{command} {key}\n"))
}

pub(crate) fn run(command: Command) {
    let result = match command {
        Command::Server => run_server(),
        Command::Input(key) => run_key("input", &key),
        Command::Down(key) => run_key("down", &key),
        Command::Up(key) => run_key("up", &key),
        Command::Reset => run_client("reset\n".into()),
        Command::Help | Command::Available => return,
    };
...
mod tests {
    ...
    #[test]
    fn rejects_unknown_cli_key_before_socket_access() {
        assert_eq!(
            run_key("input", "not-a-key").unwrap_err(),
            "unknown key or button: not-a-key"
        );
    }
    ...
    #[test]
    fn wire_parser_accepts_aliases_and_rejects_malformed_input() {
        for name in [
            "mouseleft",
            "mouse-left",
            "mouse1",
